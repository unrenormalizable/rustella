...
;===============================================================================
    ...
    ; 2014.07.05 - swapped GRP0 and GRP1 lines in the 2LK.  That gives us a
...
;---------------------------------------   
    ...
    ; prime GRP1 so player1 can appear on topmost scanline of the Arena        
        lda #HUMAN_HEIGHT-1 ; 2  9 - height of player0 graphics, 
        dcp Player0Draw     ; 5 14 - Decrement Player0Draw and compare with height
        bcs DoDrawGrp0pre   ; 2 16 - (3 17) if Carry is Set, then player0 is on current scanline
...
DoDrawGrp0pre:              ;   17 - from bcs DoDrawGRP0pre
        lda (Player0Ptr),y  ; 5 22 - load the shape for player0
        sta GRP0            ; 3 25 - @0-22, update player0 graphics
        dey                 ; 2 27
...
ArenaLoop:                  ;   13 - from bpl ArenaLoop
    ...
    ; this precalculates data that's used on line 1 of the 2LK
        ...
        dcp Player1Draw     ; 5 20 - Decrement Player1Draw and compare with height
        bcs DoDrawGrp1      ; 2 22 - (3 23) if Carry is Set, then player1 is on current scanline
        lda #0              ; 2 24 - otherwise use 0 to turn off player1
        .byte $2C           ; 4 28 - $2C = BIT with absolute addressing, trick that
                            ;        causes the lda (Player1Ptr),y to be skipped
DoDrawGrp1:                 ;   23 - from bcs DoDrawGrp1
        lda (Player1Ptr),y  ; 5 28 - load the shape for player1
        sta WSYNC           ; 3 31
;---------------------------------------
...
OverScan:
    ...
    ; generating and adjust accordingly.
        lda #32     ; set Score+1 for 27 scanlines, 32 = ((27 * 76) / 64)
