...
InitSystem:
    ...
    ; set starting location of player0 and player1 objects
        ldx #10
        stx ObjectX
        stx REFP1       ; bit D3 is on, so reflect player1
        ldx #142
        stx ObjectX+1
        ldx #$63
        stx ObjectY
...
;---------------------------------------   
    ...
    ; scanline of the Arena
        ...
        ldx #0              ; 2  9 - used for drawing playfield
...
SkipX:                      ;   35 - use 35 as it's the longest path here        
    ...
    ; this precalculates data that's used on line 1 of the 2LK
        lda #HUMAN_HEIGHT-1 ; 2 37 - height of the humanoid graphics, subtract 1 due to starting with 0
        dcp Player1Draw     ; 5 42 - Decrement Player1Draw and compare with height
        bcs DoDrawGrp1      ; 2 44 - (3 45) if Carry is Set, then player1 is on current scanline
        lda #0              ; 2 46 - otherwise use 0 to turn off player1
        .byte $2C           ; 4 50 - $2C = BIT with absolute addressing, trick that
                            ;        causes the lda (Player1Ptr),y to be skipped
DoDrawGrp1:                 ;   45 - from bcs DoDrawGrp1
        lda (Player1Ptr),y  ; 5 50 - load the shape for player1
        sta WSYNC           ; 3 53
;---------------------------------------
